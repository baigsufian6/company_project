        ...
        <section id='about' className={`${darkMode ? 'dark bg-black' : 'light bg-transparent'} w-full m-auto lg:px-40 px-10 py-20 grid lg:grid-cols-2 grid-cols-1 justify-center items-center gap-10`}>
            ...
            <div className="flex flex-col justify-center items-start gap-8">
                ...
                <p data-aos="zoom-in" data-aos-delay="400" className="text-xl text-gray-600 dark:text-white text-justify">
                    Our journey began with a vision to create spaces that people love. Today, our happy customers are a testament 
                    to our dedication. From the initial consultation to the final handover,
                     we are with you every step of the way, ensuring a smooth and satisfying experience.
                </p>
                <button className="bg-red-600 dark:bg-red-700 hover:bg-black dark:hover:bg-white
                 dark:hover:text-black text-lg p-4 w-full text-white font-semibold rounded-xl cursor-pointer
                  transform hover:scale-110 transition-transform duration-300" onClick={() => navigate('./Detail')}>READ MORE</button>
            </div>
        </section>
    );
}
